...
/*   Updated: 2022/10/11 13:38:01 by vduchi           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	key_hook(int keycode, t_all *vars)
{
	...
	{
		vars->fractol.status = 1;
		if (vars->fractol.zoom <= 1)
			vars->fractol.left_right -= 0.5;
		else
...
int	create_window(t_all *vars)
{
	...
	vars->win = mlx_new_window(vars->mlx, vars->fractol.max_x, vars->fractol.max_y, "Mandelbrot");
